...
current_ref_pose_as_start_(false)
{
	...
	sub_odom_ = nh_.subscribe("/uav/odometry", 1, &TrajectoryGenerator::uavOdomCallback, this);
...
template <int _N>
void TrajectoryGenerator<_N>::uavOdomCallback(const nav_msgs::Odometry::ConstPtr& pose_msgs ){
  tf::poseMsgToEigen(pose_msgs->pose.pose, ref_pose_se3_);
  tf::vectorMsgToEigen(pose_msgs->twist.twist.linear, current_velocity_);
  tf::vectorMsgToEigen(pose_msgs->twist.twist.angular, current_angular_velocity_);
//   cur_position << pose_msgs->pose.pose.position.x, pose_msgs->pose.pose.position.y, pose_msgs->pose.pose.position.z;
  
//   geometry_msgs::Quaternion quat = pose_msgs->pose.pose.orientation;
//   yaws_= tf::getYaw(quat);
}
